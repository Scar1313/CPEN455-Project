...
def my_sample(model, gen_data_dir, sample_batch_size=25, obs=(3,32,32), sample_op=sample_op):
    for label in my_bidict:
        ...
        samples = sample(model, sample_batch_size, obs, sample_op, label=my_bidict[label])
        samples = rescaling_inv(samples)
        save_images(samples, os.path.join(gen_data_dir), label=label)
# End of your code
...
if __name__ == "__main__":
    ...
    model_path = os.path.join(os.path.dirname(__file__), 'models/pcnn_cpen455_from_scratch_100.pth')
